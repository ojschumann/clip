...
void ProjectionPlane::on_actionTestAuto_triggered() {
  if (projector->getLaueImage()) {
    QSize imgSize = projector->getLaueImage()->transformedSize().toSize()/4;
    QImage img = projector->getLaueImage()->getScaledImage(imgSize, QPolygonF(QRectF(0, 0, 1, 1)));
    QImage out(361, 361, QImage::Format_ARGB32);
    out.fill(qRgb(0,0,128));

    QVector<double> pix(out.width()*out.height());

    for (int ix=0; ix<out.width(); ix++) {
      cout << "Line " << ix << endl;
      for (int iy=0; iy<out.height(); iy++) {
        double x = (1.0*ix-180.0)/180.0;
        double y = (1.0*iy-180.0)/180.0;

        Vec3D z(1.0-x*x-y*y, 2.0*x, 2.0*y);
        if (z.x()<0.0) continue;
        ...
        if (r.norm()<1e-6)
          r=Vec3D(0,1,0);
        r.normalize();
        Mat3D M(z, M_PI/360.0);

        double val=0.0;
        double weight = 0.0;
        for (int i=0; i<360; i++) {
          bool ok=true;
          QPointF p = projector->normal2det(r, ok);
          if (ok) {
            p = projector->det2img.map(p);
            int imgX = qRound(p.x()*img.width());
            int imgY = qRound(p.y()*img.height());
            if (imgX>=0 && imgX<img.width() && imgY>=0 && imgY<img.height()) {
              weight += 1;
              double v = qBlue(img.pixel(imgX, imgY));
              //val += exp(v/256.0);
              val += v;
            }
          }
    ...
    for (int i=0; i<pix.size(); i++) {
      if (i==0 || pmin>pix[i]) pmin = pix[i];
      if (i==0 || pmax<pix[i]) pmax = pix[i];
    }
    for (int i=0; i<pix.size(); i++) {
      int x = i%361;
      int y = i/361;
      int v = static_cast<int>(255.0*(pix[i]-pmin)/(pmax-pmin));
      out.setPixel(x,y,qRgb(v,v,v));
    }
    out.save(QString("test.png"));
  }
}
